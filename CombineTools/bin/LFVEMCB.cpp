...
int main(int argc, char* argv[]){
 ...
 string label = "h#rightarrow e#tau_{#mu} M_{col} fit";
 string lumi = "136.7";
 ...
 const Int_t ny = 5;
 string channels[ny] = {"e#tau_{#mu}", "e#tau_{#mu}, VBF","e#tau_{#mu}, 2 Jets","e#tau_{#mu}, 1 Jet", "e#tau_{#mu}, 0 Jets"};
...
double ComputeSumYLimit(double BranchingRatio = 0.1){
  ...
  // BR(h->etau) = Width(h->etau) / ( Width(h->etau) + SM_Higgs_Width)
  // Width(h->etau) = mh / 8Pi * (|Y(etau)|**2 + |Y(taue)|**2)
  // (|Y(etau)|**2 + |Y(taue)|**2) = (BR * SM_Higgs_Width) / ()*(1- BR)

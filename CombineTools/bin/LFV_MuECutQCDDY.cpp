...
int main(int argc, char* argv[]){
  ...
  string lumi="";
  string inputFile="shapesMuEQCDDY";
  ...
  cb.cp().process({"Zothers"})
    .AddSyst(cb, "norm_Zothers_mutaue_$BIN", "lnN", SystMap<>::init(1.05));

  cb.cp().process({"ZTauTau"})
    .AddSyst(cb, "norm_ZTauTau_mue", "lnN", SystMap<>::init(1.04));
  cb.cp().process({"Zothers"})
    .AddSyst(cb, "norm_ZTauTau_mutaue_$BIN", "lnN", SystMap<>::init(1.05));
  ...
  cb.cp().process({"W"})
    .AddSyst(cb, "norm_w_mutaue_$BIN", "lnN", SystMap<>::init(1.05));
  cb.cp().process({"WG"})
    .AddSyst(cb, "norm_wg", "lnN", SystMap<>::init(1.10));
  ...
  cb.cp().process({"EWK"})
    .AddSyst(cb, "norm_EWK", "lnN", SystMap<>::init(1.10));
  cb.cp().process({"EWK"})
    .AddSyst(cb, "norm_EWK_mutaue_$BIN", "lnN", SystMap<>::init(1.05));

  // Luminosity, Pileup, Prefiring
  cb.cp().process(ch::JoinStr({sig_procs, {"ggH_htt", "qqH_htt", "ggH_hww", "qqH_hww", "vH_htt", "Zothers", "TT", "T", "EWK", "Diboson", "W", "WG", "ZTauTau"}}))
  ...
  // Recoil Uncertainty
  cb.cp().process(ch::JoinStr({sig_procs, {"ggH_htt", "qqH_htt", "ggH_hww", "qqH_hww", "Zothers", "EWK", "W", "ZTauTau"}}))
  ...
  cb.cp().process(ch::JoinStr({{"Zothers"}}))
    .AddSyst(cb, "CMS_DYpTreweight_13TeV", "shape", SystMap<>::init(1.0));

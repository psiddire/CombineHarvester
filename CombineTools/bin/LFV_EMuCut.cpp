...
int main(int argc, char* argv[]){
  ...
  string outputFile="HETau_etaumu_2018_m125.input";
  string dirInput="";
  bool binbybin=false;
  string name="def";
  ...
  cb.AddObservations({"*"}, {"HETau"}, {"2018"}, {"etaumu"}, cats);

  vector<string> bkg_procs = {"ggH_htt", "qqH_htt", "ggH_hww", "qqH_hww", "Zothers", "EWK", "W", "TT", "T", "Diboson", "QCD", "ZTauTau"};
  cb.AddProcesses({"*"}, {"HETau"}, {"2018"}, {"etaumu"}, bkg_procs, cats, false);

  vector<string> sig_procs = {"LFVGG", "LFVVBF"};
  cb.AddProcesses(masses, {"HETau"}, {"2018"}, {"etaumu"}, sig_procs, cats, true);
  ...
  // Trigger
  cb.cp().process(ch::JoinStr({sig_procs, mc_procs}))
    .AddSyst(cb, "CMS_eff_trigger_etaumu_2018", "lnN", SystMap<>::init(1.02));
  cb.cp().process({"ZTauTau"})
    .AddSyst(cb, "CMS_eff_trigger_emb_etaumu_2018", "lnN", SystMap<>::init(1.02));
  cb.cp().process({"ZTauTau"})
  ...
  cb.cp().process({"QCD"})
    .AddSyst(cb, "CMS_qcd_0jet_rate_2018", "shape", SystMap<>::init(1.0));
  cb.cp().process({"QCD"})
    .AddSyst(cb, "CMS_qcd_1jet_rate_2018", "shape", SystMap<>::init(1.0));
  cb.cp().process({"QCD"})
    .AddSyst(cb, "CMS_qcd_2jet_rate_2018", "shape", SystMap<>::init(1.0));
  cb.cp().process({"QCD"})
    .AddSyst(cb, "CMS_qcd_0jet_shape_2018", "shape", SystMap<>::init(1.0));
  cb.cp().process({"QCD"})
    .AddSyst(cb, "CMS_qcd_1jet_shape_2018", "shape", SystMap<>::init(1.0));
  cb.cp().process({"QCD"})
    .AddSyst(cb, "CMS_qcd_2jet_shape_2018", "shape", SystMap<>::init(1.0));
  cb.cp().process({"QCD"})

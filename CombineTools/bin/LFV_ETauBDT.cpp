...
int main(int argc, char* argv[]){
  ...
  // Trigger
  cb.cp().process(ch::JoinStr({sig_procs, mc_procs}))
    .AddSyst(cb, "CMS_eff_trigger_etauh_2018", "shape", SystMap<>::init(1.0));
  cb.cp().process({"ZTauTau"})
    .AddSyst(cb, "CMS_eff_trigger_emb_etauh_2018", "shape", SystMap<>::init(1.0));
  cb.cp().process({"ZTauTau"})
  ...
  cb.cp().process({"Fakes"})
    .AddSyst(cb, "CMS_eFR_p0_2018", "shape", SystMap<>::init(1.0));
  cb.cp().process({"Fakes"})
    .AddSyst(cb, "CMS_eFR_p1_2018", "shape", SystMap<>::init(1.0));
